...
class ColumnMapper:
    ...
    HEADER_MAP: Dict[str, str] = {"organization": "organisation",
        "organisation": "organisation", "name of": "organisation","type": "breach_type","year": "breach_date","date": "breach_date","records": "records_exposed","state": "state","city": "city", "industry": "industry","sector": "industry",}
...
class BreachScraper(BaseExtractor):
    ...
    def __init__(self,kafka_producer=None,output_dir: str = ".",
        max_pages: int = DEFAULT_MAX_PAGES,sleep_sec: float = DEFAULT_SLEEP_SEC,
        target_records: int = DEFAULT_TARGET_RECORDS,batch_size: int = DEFAULT_BATCH_SIZE,):
    ...
    def _fetch_records(self):
        ...
        for row in raw_rows:
            ...
            records.append({"organisation": organisation,
        ...
        if original_count < self._target_records:
            ...
            self.logger.info(f"Expanded breach source from {original_count:,} rows to {len(records):,} records for local analysis")
        else:
    ...
    def _expand_records( self, records: List[Dict[str, Any]], target_count: int,) -> List[Dict[str, Any]]:
        ...
        for index in range(target_count):
            ...
            if index >= source_count:
                expanded_record["organisation"] = (f"{source_org} [synthetic {index + 1:06d}")
...
if __name__ == "__main__":scraper = BreachScraper(
        kafka_producer=None,
        output_dir=".",
        max_pages=5)
...
print(f"Done. {total:,} breach records written to breach_raw.json")
