...
class PostgresResource(ConfigurableResource):
    ...
    def get_connection(self):
        ...
        return psycopg2.connect(host=self.host,  port=self.port,dbname=self.dbname,user=self.user, password=self.password,connect_timeout=10)
...
class PipelineConfig(ConfigurableResource):
    """ General pipeline configuration. Set quick_test=True in dagster UI/ run config to do a fast test without pulling all NVD records."""
